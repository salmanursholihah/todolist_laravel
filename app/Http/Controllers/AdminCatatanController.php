    ...
    public function index(Request $request)
{
    ...
    // Ambil bulan & tahun dari request (kalau ada)
    $bulan = $request->input('bulan', now()->format('m'));
    $tahun = $request->input('tahun', now()->format('Y'));

    // Query builder (JANGAN pakai get() dulu)
    $query = Catatan::with(['user', 'images'])
                ->whereMonth('created_at', $bulan)
                ->whereYear('created_at', $tahun)
                ->latest();

    // Eksekusi dengan paginate (WAJIB TERAKHIR)
       $catatans = Catatan::with(['user','images'])
        ->latest()
        ->paginate(10);

    return view('admin.catatans.index', compact(
        'catatans',
        'users',
        'bulan',
        'tahun'
